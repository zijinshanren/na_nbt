...
fn test_write_list_fallback_primitives() {
    ...
    assert_eq!(
        &written[0..8],
        &[0x09, 0x00, 0x00, 0x02, 0x02, 0x00, 0x00, 0x00]
    ); // LE count
    // Check data LE
...
fn test_write_list_fallback_complex() {
    ...
    // s1 len (LE)
    assert_eq!(
        &written[offset..offset + 2],
        &(s1.len() as u16).to_le_bytes()
    );
    offset += 2;
...
fn test_write_list_fallback_arrays() {
    ...
    let offset = 8;
    assert_eq!(
        &written[offset..offset + 4],
        &(b1.len() as u32).to_le_bytes()
    );
...
fn test_write_compound_fallback_all_types() {
    ...
    assert_eq!(
        comp.get("ia").unwrap().as_int_array().unwrap()[0].get(),
        0x12345678
    );
    assert_eq!(
        comp.get("la").unwrap().as_long_array().unwrap()[0].get(),
        0x1122334455667788
    );
}
