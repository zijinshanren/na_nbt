...
fn test_int_array_vs_vec_i32_size() {
    ...
    assert!(int_array_bytes.len() <= vec_bytes.len(), 
...
fn test_int_array_tag_is_correct() {
    ...
    assert_eq!(bytes[3], Tag::IntArray as u8, "Field tag should be IntArray (11)");
}
...
fn test_long_array_tag_is_correct() {
    ...
    assert_eq!(bytes[0], Tag::Compound as u8);
    assert_eq!(bytes[3], Tag::LongArray as u8, "Field tag should be LongArray (12)");
}
...
fn test_byte_array_tag_is_correct() {
    ...
    assert_eq!(bytes[0], Tag::Compound as u8);
    assert_eq!(bytes[3], Tag::ByteArray as u8, "Field tag should be ByteArray (7)");
}
...
fn test_vec_of_structs_with_arrays() {
    ...
    let original = Container {
        items: vec![
            Item { id: 1, data: vec![10, 20] },
            Item { id: 2, data: vec![30, 40, 50] },
        ],
    };
