...
fn test_index_mut_access() {
    ...
    if let Some(mut mv) = owned.get_mut(key.clone()) { // Pass String by value
        mv.set_int(30);
    }
    ...
    if let Some(mut list) = owned.get_mut("li") {
        if let Some(mut el) = list.get_mut(0_usize) { // Pass usize by value
            el.set_int(100);
        }
        if let Some(mut el) = list.get_mut(&1_usize) { // Pass &usize
            el.set_int(200);
        }
    }
    ...
    let idx = 0_usize;
    if let Some(mut list) = owned.get_mut("li") {
        if let Some(mut el) = list.get_mut(&&idx) { // Pass &&usize
             el.set_int(101);
        }
    }
