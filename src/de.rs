...
/*
 * Vec<Compound> -> List<Wrap<Compound>>
 * Vec<T: !Compound> -> List<Wrap<T>>
 * Vec<Compound> + #direct -> List<Compound>
 *
 * Now given: List[Compound], how to deserialize it?
 */
...
impl<'a, 'de, O: ByteOrder> MapAccess<'de> for CompoundAccess<'a, 'de, O> {
    ...
    {
        check_bounds!(1, self.deserializer.input);
        let tag_id = TagID::from_u8(self.deserializer.input[0])?;
...
impl<'a, 'de, O: ByteOrder> SeqAccess<'de> for TupleDeserializer<'a, 'de, O> {
        ...
        T: de::DeserializeSeed<'de>,
    {
        ...
        if name_len != 0 {
            cold_path();
            return Err(Error::MSG("Expected empty name in tuple element".into()));
        }
        // Skip tag_id and empty name
        self.deserializer.input = &self.deserializer.input[1 + 2..];
        self.deserializer.current_tag = tag_id;
...
impl<'de> VariantAccess<'de> for UnitVariantAccess {
    ...
    {
        cold_path();
        Err(Error::MSG("Expected unit variant".into()))
    }
...
impl<'a, 'de, O: ByteOrder> VariantAccess<'de> for VariantValueDeserializer<'a, 'de, O> {
    ...
    fn unit_variant(self) -> Result<()> {
        cold_path();
        Err(Error::MSG("Expected newtype/tuple/struct variant".into()))
    }
